...
from jinja2 import Template  # se preferir, pode fazer string format manual
import tempfile

def gerar_pdf_recibo(dados_venda, itens, numero_recibo):
    """
    dados_venda: dict com chaves: cliente_nome, data_venda, valor_total, valor_pago, valor_devendo, observacoes
    ...
    <head>
        ...
        <style>
            ...
            .container { max-width: 800px; margin: 0 auto; background: #fff; padding: 20px; border: 1px solid #ddd; }
            .header { text-align: center; margin-bottom: 20px; }
            .logo { max-width: 150px; margin-bottom: 10px; }
            .empresa { font-size: 24px; font-weight: bold; color: #333; }
            .titulo-recibo { font-size: 18px; margin: 10px 0; color: #555; }
            .info { margin-bottom: 20px; }
            .info td { padding: 4px; }
            table { width: 100%; border-collapse: collapse; margin: 15px 0; }
            th, td { border: 1px solid #ccc; padding: 8px; text-align: left; }
            th { background-color: #f2f2f2; }
            .totais { width: 300px; margin-left: auto; margin-top: 20px; }
            .totais td { border: none; padding: 4px; }
            .total-final { font-weight: bold; font-size: 1.1em; }
            .agradecimento { text-align: center; margin-top: 30px; font-style: italic; color: #777; }
    ...
    <body>
        <div class="container">
            <div class="header">
                <!-- Logo opcional: descomente se tiver uma imagem local -->
                <!-- <img class="logo" src="data:image/png;base64,{{ logo_base64 }}" alt="Logo"> -->
                <div class="empresa">Estoque Ovos Pro</div>
                <div class="titulo-recibo">RECIBO DE VENDA</div>
            </div>

            <table class="info">
                <tr><td width="120"><strong>Nº Recibo:</strong></td><td>{{ numero_recibo }}</td></tr>
                <tr><td><strong>Data:</strong></td><td>{{ data_venda }}</td></tr>
                <tr><td><strong>Cliente:</strong></td><td>{{ cliente_nome }}</td></tr>
                <tr><td><strong>Observações:</strong></td><td>{{ observacoes }}</td></tr>
            </table>

            <table>
                <thead>
                    <tr><th>Produto</th><th>Qtd</th><th>Preço Unit. (R$)</th><th>Desc. Unit. (R$)</th><th>Subtotal (R$)</th></tr>
                </thead>
                <tbody>
                    {% for item in itens %}
                    <tr>
                        ...
                        <td>{{ "%.2f"|format(item.preco_unit) }}</td>
                        <td>{{ "%.2f"|format(item.desconto_unit) }}</td>
            ...
            <table class="totais">
                <tr><td>Valor Bruto:</td><td>{{ "%.2f"|format(valor_bruto) }}</td></tr>
                <tr><td>Desconto Total:</td><td>{{ "%.2f"|format(desconto_total) }}</td></tr>
                <tr class="total-final"><td>Valor Final:</td><td>{{ "%.2f"|format(valor_total) }}</td></tr>
                <tr><td>Valor Pago:</td><td>{{ "%.2f"|format(valor_pago) }}</td></tr>
                <tr><td>Saldo Devedor:</td><td>{{ "%.2f"|format(valor_devendo) }}</td></tr>
            </table>
    ...
    # Se você tiver uma logo, pode carregar como base64 e passar no contexto
    ...
    html_content = template.render(
        ...
        data_venda=dados_venda['data_venda'].strftime('%d/%m/%Y'),
        ...
        valor_devendo=dados_venda['valor_devendo']
    )
                            ...
                            with col_btn1:
                                if st.button("✅ Confirmar e Registrar", type="primary", use_container_width=True):
                                    try:
                                        ...
                                        # ========== AQUI - APÓS A TRANSAÇÃO (venda confirmada) ==========
                                        # Gerar o PDF do recibo (os dados estão em dados_venda e carrinho)
                                        pdf_bytes = gerar_pdf_recibo(
                                            dados_venda=dados_venda,
                                            itens=st.session_state.carrinho,
                                            numero_recibo=dados_venda.get('numero_recibo', 'N/A')
                                        )

                                        # Opcional: Oferecer download (pode ser removido depois)
                                        st.download_button(
